<template>
  <template v-for="menuOption in props.menuOptions" :key="menuOption.id">
    <template v-if="!menuOption.children?.length || [MenuType.Page, MenuType.Tab].includes(menuOption.type) ">
      <el-menu-item :index="menuOption.path">
        <el-icon  v-if="menuOption.meta!.icon && menuOption.meta!.icon.length > 0">
          <template v-if="Icons[menuOption.meta!.icon]">
            <component :is="Icons[menuOption.meta!.icon]" />
          </template>
          <template v-else>
            <SVGIcon :name="menuOption.meta!.icon"/>
          </template>
        </el-icon>
        <template #title>
