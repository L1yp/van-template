...
export function getTreeItemPath<T extends Tree>(src: T[], keyField: keyof T, key: T[keyof T], includeThis?: boolean): T[] {
  // 深度遍历
  const result: T[] = []
  DFSTree(src, keyField, key, result, includeThis)
  return result.reverse()
}

function DFSTree<T extends Tree>(src: T[], keyField: keyof T, key: T[keyof T], result: T[], includeThis?: boolean): boolean {
  for (let item of src) {
    if (item[keyField] === key) {
      includeThis && result.push(item)
      return true
    }
    if (item.children?.length) {
      const ret = DFSTree(item.children, keyField, key, result, includeThis)
      if (ret) {
        result.push(item)
        return true
      }
